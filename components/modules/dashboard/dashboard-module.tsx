...
export default function DashboardModule() {
  ...
  const fetchDashboardData = async () => {
    try {
      const response = await apiClient.request('/dashboard/overview', {
        method: 'GET'
      })

      if (response.data) {
        setDashboardData({
          stock_metrics: response.data.stock_metrics || {
            disponible: 0,
            comprometido: 0,
            agotados: 0,
            total_productos: 0
          },
          financial_metrics: response.data.financial_metrics || {
            valor_inventario: 0,
            valor_venta_potencial: 0
          },
          critical_products: response.data.critical_products || [],
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{(dashboardData?.stock_metrics?.disponible || 0).toLocaleString()}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{(dashboardData?.stock_metrics?.comprometido || 0).toLocaleString()}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{(dashboardData?.stock_metrics?.agotados || 0).toLocaleString()}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">${((dashboardData?.financial_metrics?.valor_inventario || 0) / 1000).toFixed(1)}K</div>
