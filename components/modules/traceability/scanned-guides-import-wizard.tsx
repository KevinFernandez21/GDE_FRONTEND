...
const REQUIRED_COLUMNS = ['numero_guia', 'tipo_movimiento']
const VALID_MOVEMENTS = ['salida', 'entrada']

export default function ScannedGuidesImportWizard({ isOpen, onClose, onImportComplete, onCancel }: ImportWizardProps) {
  ...
  // Parse CSV file - Basic client-side parsing
  ...
  const validateFile = async (file: File) => {
    ...
    try {
      ...
      // For Excel files, skip validation and go directly to import
      if (fileExt === '.xlsx' || fileExt === '.xls') {
        const validationData: ValidationResult = {
          success: true,
          valid_for_import: true,
          file_info: {
            rows: 0, // Unknown until backend processes
            columns: 0,
            file_type: 'excel'
          },
          column_validation: {
          ...
          data_validation: {
            critical_errors: [],
            warnings: [{
              message: "Los archivos Excel serán validados y procesados por el backend",
              affected_rows: []
            }],
            total_errors: 0
          },
        ...
        setStep("review")
        toast.info("Archivo Excel detectado", {
          description: "El archivo será procesado por el backend. Puedes continuar con la importación."
        })
        setIsValidating(false)
        return
      }

      // For CSV files, do full validation
      ...
      const unknownColumns = fileColumns.filter(col => !REQUIRED_COLUMNS.includes(col))
      ...
      data.forEach((row, index) => {
        ...
        if (!row.numero_guia || String(row.numero_guia).trim() === '') {
          errors.push({
            message: `Fila ${rowNum}: Número de guía es requerido`,
            affected_rows: [rowNum]
          })
        }

        if (!row.tipo_movimiento || String(row.tipo_movimiento).trim() === '') {
          errors.push({
            message: `Fila ${rowNum}: Tipo de movimiento es requerido`,
            affected_rows: [rowNum]
          })
        } else {
          const movimiento = String(row.tipo_movimiento).trim().toLowerCase()
          if (!VALID_MOVEMENTS.includes(movimiento)) {
            errors.push({
              message: `Fila ${rowNum}: Tipo de movimiento inválido: "${row.tipo_movimiento}". Debe ser "salida" o "entrada"`,
              affected_rows: [rowNum]
            })
          }
        }
      ...
      if (validationData.valid_for_import) {
        toast.success("Validación exitosa", {
          description: `${data.length} guías escaneadas válidas listas para procesar`
        })
      } else {
    ...
    try {
      ...
      if (response.data || response.success) {
        ...
        toast.success("Procesamiento exitoso", {
          description: `${response.data?.total_processed || response.total_processed || 0} guías procesadas exitosamente`
        })
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col m-4">
        ...
        <div className="flex-shrink-0 p-6 border-b">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold">Subir Guías Escaneadas</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            ...
            {step === "upload" && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Sube un archivo CSV o Excel con guías escaneadas
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  >
                    ...
                    />
                    <Badge variant="secondary">CSV o Excel</Badge>
                  </div>
                  ...
                  <Alert>
                    ...
                    <AlertDescription className="space-y-2">
                      ...
                      <ul className="list-disc list-inside text-sm space-y-1">
                        <li><strong>numero_guia</strong>: Código de la guía</li>
                        <li><strong>tipo_movimiento</strong>: "salida" o "entrada"</li>
                      </ul>
                      <p className="text-xs text-muted-foreground mt-2">
                        <strong>Nota:</strong> El sistema actualizará automáticamente los estados de las guías según el tipo de movimiento.
                      </p>
                    </AlertDescription>
                  </Alert>
                </CardContent>
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      ...
                      {validationResult.valid_for_import ? (
                        <Badge className="bg-green-500">✓ Listo para Procesar</Badge>
                      ) : (
            ...
            {step === "import" && (
              <Card>
                <CardHeader>
                  <CardTitle>Procesando Guías</CardTitle>
                  <CardDescription>
                    Actualizando estados de guías...
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-center py-12">
            ...
            {step === "complete" && importResult && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Las guías han sido procesadas exitosamente
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div className="text-center p-4 border rounded-lg">
                      <div className="text-3xl font-bold">{importResult.total_processed || 0}</div>
                      <div className="text-sm text-muted-foreground">Total Procesado</div>
                    </div>
                    <div className="text-center p-4 border rounded-lg bg-green-50">
                      <div className="text-3xl font-bold text-green-600">{importResult.imported_count || importResult.matches || 0}</div>
                      <div className="text-sm text-muted-foreground">Encontradas</div>
                    </div>
                    <div className="text-center p-4 border rounded-lg bg-yellow-50">
                      <div className="text-3xl font-bold text-yellow-600">{importResult.skipped_count || importResult.not_found || 0}</div>
            ...
            <div className="flex gap-2">
              ...
              {step === "review" && validationResult && validationResult.valid_for_import && (
                ...
                >
                  ...
                  {isImporting ? "Procesando..." : "Procesar Guías"}
                </Button>
              )}
